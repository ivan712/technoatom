class valuta:
    ...
    def convert(amount,name1,name2):
        koeff_euro_rub = 75.84 
        koeff_doll_rub = 67.52
        koeff_chf_rub = 71.62
        koeff_try_rub = 11.06
        if name1 == 'RUB':
            conv_amount = amount
            ...
            conv_amount = amount*coeff_try_rub
    ...
    def __add__(self,other):
        if type(self) == type(other):
            if self.name == '':
                return valuta(self.amount + other.amount, other.name)
            elif other.name == '':
                return valuta(self.amount + other.amount, self.name)
            return valuta(round(self.amount + self.convert(other.amount,self.name,other.name),2),self.name)
        else:
